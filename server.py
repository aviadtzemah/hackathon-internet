...
class Match:
        ...
        while self.waiting_for_connections:
            try:
                ...
                if not name_of_the_client: # in case something happened and the client was lost, close the connection
                    ...
                    self.connected_clients.append((num_of_connected_clients, name_of_the_client, client_socket, client_address, (random_numer % 2) + 1))
            except timeout:
                pass    
            
        print(self.waiting_for_connections)              
    ...
    def wait_for_clients(self):
        ...
        message = b'\xfe\xed\xbe\xef\x02\x08\x1d'
...
try: 
    tcp_socket_for_server.bind((host, port))
    tcp_socket_for_server.settimeout(1)
    tcp_socket_for_server.listen() # listening for incoming clients
except error as e:
    print(f'failed to bind the socket: {str(e)}') #TODO should i do something else?


while True:
    current_match = Match(10, 10, broadcast_port, tcp_socket_for_server)# TODO put the args
